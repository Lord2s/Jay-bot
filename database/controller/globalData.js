...
module.exports = async function (databaseType, globalModel, fakeGraphql) {
      ...
      switch (mode) {
        ...
        case "update": {
          ...
          if (Array.isArray(path) && Array.isArray(data)) {
            ...
            for (const key in data) dataWillChange[key] = data[key];
          }
  ...
  async function create_(key, data) {
    return new Promise(async (resolve, reject) => {
      ...
      if (Object.keys(data).some(key => !['key', 'data'].includes(key))) {
        const err = new Error(`The second argument (data) must be an object with keys "key" and "data"`);
        err.name = "INVALID_TYPE";
        return reject(err);
      }
      ...
      const queue = new Promise(async function (resolve_, reject_) {
        ...
        if (findIndex !== -1) creatingGlobalData.splice(findIndex, 1);
      });
      ...
      messageQueue.push(async function () {
        try {
          ...
          if (query)
            ...
            else dataReturn = dataReturn.map(uData => fakeGraphql(query, uData));
  ...
  async function get_(key, path, defaultValue, query) {
    ...
    if (!dataReturn) {
      ...
      if (defaultValue) {
        if (path)
          ...
          else if (typeof path === "string") _.set(createData, path, defaultValue);
          else _.times(path.length, i => _.set(createData, path[i], defaultValue[i]));
        else _.set(createData, "data", defaultValue);
    ...
    if (query)
      ...
      else dataReturn = fakeGraphql(query, dataReturn);
    ...
    if (path)
      ...
      else if (typeof path === "string") return _.cloneDeep(_.get(dataReturn, path, defaultValue));
      else return _.cloneDeep(_.times(path.length, i => _.get(dataReturn, path[i], defaultValue[i])));
